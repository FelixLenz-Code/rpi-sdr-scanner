...
_BL_SYSFS = '/sys/class/backlight/fb_ili9486/bl_power'  # 0=an, 1=aus
...
class DisplayUI:
    def __init__(self, scanner, debug: bool = False,
        ...
        # Menü-Zustand
        self._bl_on = True   # Backlight AN/AUS
    ...
    def _set_backlight(on: bool) -> None:
        """Schaltet Display-Backlight via sysfs (fbtft: bl_power 0=an, 1=aus)."""
        try:
            with open(_BL_SYSFS, 'w') as f:
    ...
    def _menu_select(self):
        ...
        if action == "__BRIGHTNESS__":
            self._bl_on = not self._bl_on
            self._set_backlight(self._bl_on)
            self._draw()
            return
